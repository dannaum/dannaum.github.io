		...
		$(window).scroll(function () {
			...
			if ($(".fadeup1").isInViewport()) {
				...
				if (!viewedCareersGraph) {
					...
					document.querySelector("#all-chart").addEventListener("click", function () {
						chart.destroy();
						chart = new Chart(ctx, config);
					});
					document.querySelector("#first-chart").addEventListener("click", function () {
						chart.destroy();
						chart = new Chart(ctx, configOne);
					});
					document.querySelector("#second-chart").addEventListener("click", function () {
						chart.destroy();
						chart = new Chart(ctx, configTwo);
					});
					document.querySelector("#third-chart").addEventListener("click", function () {
						chart.destroy();
						chart = new Chart(ctx, configThree);
					});
