...
function checkCurrencies() {
    ...
    if (country === 'US') {
        for (var i = 0; i < paragraphCurrency.length; i++) {
            if (paragraphCurrency[i].innerHTML.indexOf('€50,000') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€50,000', '$125,000');
            }
            if (paragraphCurrency[i].innerHTML.indexOf('€2 billion') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€2 billion', '$2 billion');
            }
        }
        $('#email-form').attr('action', 'https://us.moonfare.com/registration');
    }
    if (country === 'CH') {
        for (var i = 0; i < paragraphCurrency.length; i++) {
            if (paragraphCurrency[i].innerHTML.indexOf('€50,000') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€50,000', 'CHF 50,000');
            }
            if (paragraphCurrency[i].innerHTML.indexOf('€2 billion') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€2 billion', 'CHF 2 billion');
            }
        }
    }
    if (country === 'GB') {
        for (var i = 0; i < paragraphCurrency.length; i++) {
            if (paragraphCurrency[i].innerHTML.indexOf('€50,000') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€50,000', '£50,000');
            }
            if (paragraphCurrency[i].innerHTML.indexOf('€2 billion') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€2 billion', '£1.7 billion');
            }
        }
    }
    if (country === 'HK') {
        for (var i = 0; i < paragraphCurrency.length; i++) {
            if (paragraphCurrency[i].innerHTML.indexOf('€50,000') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€50,000', '$60,000');
            }
            if (paragraphCurrency[i].innerHTML.indexOf('€2 billion') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€2 billion', '$2 billion');
            }
    ...
    if (country === 'SG') {
        for (var i = 0; i < paragraphCurrency.length; i++) {
            if (paragraphCurrency[i].innerHTML.indexOf('€50,000') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€50,000', 'SGD 80,000');
            }
            if (paragraphCurrency[i].innerHTML.indexOf('€2 billion') > -1) {
                paragraphCurrency[i].innerHTML = paragraphCurrency[i].innerHTML.replace('€2 billion', 'SGD 2.9 billion');
            }
        }
    }
}
