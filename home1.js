...
$('.past-future_slider-slide').each(function () {
  ...
  $(this)
    .find('.past-future_single-card')
    .attr('data-testid', 'closed-funds-card-' + parent_index);
});
...
$('#ab-one_no').click(function () {
  ...
  $('#ab-one_yes').click(function () {
    $('body').css('overflow', 'hidden'),
      ...
      $('#cashflow-ab_test').css('display', 'flex'),
      $('#cashflow-ab_test').fadeIn(500),
      (responsiveAnimationHeight = $(
        '.ab-test-cf-animation-wrapper'
      ).outerHeight()),
      dragDetector();
  }),
    ...
    $(this).hasClass('opened-dd')
      ? (991 > $(window).width()
          ? ($(o).find('.pill-plus_icon').css('display', 'flex'),
            ...
            $('.ab-test_button-option-three_animated-bg').animate(
      ...
      : (991 > $(window).width()
          ? ($(o).find('.pill-plus_icon').css('display', 'none'),
            ...
            $('.ab-test_button-option-three_animated-bg').animate(
              { height: responsiveAnimationHeight + 'px', width: '100%' },
              500
            ))
          : ($('.relative-button').animate({ opacity: 0 }, 500),
            ...
            setTimeout(function () {
              $('.ab-test_button-option-three_animated-bg').animate(
                ...
                setTimeout(function () {
                  $('.minus-button').animate({ opacity: 1 }, 500),
                    $('.closed-funds_ab-content').animate({ opacity: 1 }, 500);
                }, 500);
            }, 500)),
        $(this).toggleClass('opened-dd'));
...
function dragDetector() {
  ...
  (abdd = new Dragdealer('ab-scroller', {
    ...
    animationCallback: function (e, n) {
      $('.past-future-slider-active_line.ab-test').css(
        'width',
        Math.round(100 * e) + '%'
      ),
        $('.closed-funds_ab-wrapper-mask').css('margin-left', -e * (i - o));
    },
  })),
    o < i
      ? $('.past-future-drag-component.ab-test-component').css('opacity', '1')
      : $('.past-future-drag-component.ab-test-component').css('opacity', '0');
  ...
  dd = new Dragdealer('content-scroller', {
    ...
    animationCallback: function (o, i) {
      $('.past-future-slider-active_line').css(
        ...
        $('.past-future_slider').css('margin-left', -o * (l - n + 32));
    },
  });
  var closedFundsWrapper = Math.round($('.past-future_content').outerWidth());
  var closedFundsTotalWidth = Math.round(
...
$('.ab-test_button-option').mouseover(function () {
  ...
  $('.ab-test_button-option-three_cta').mouseover(function () {
    $('.ab-test_button-option-three').removeClass('active-hover'),
      $('.ab-test_button-option-three_cta').removeClass('active-hover-cta'),
      $(this).addClass('active-hover-cta');
  }),
  $(
    '.cashflow_ab-test_close-button, .cashflow_ab-test_close-button-two, .cashflow_ab-test_close-button-three'
  ).click(function () {
    $('body').css('overflow', 'auto'), $('#cashflow-ab_test').fadeOut();
  }),
  $('#cashflow-ab_test').click(function (o) {
    o.target === o.currentTarget &&
      ($('#cashflow-ab_test').fadeOut(), $('body').css('overflow', 'auto'));
  }),
  dragDetector();
var resizeDone;
...
function doneResizing() {
  ...
  $('.ab-test_button-option-three.first').hasClass('opened-dd')
    ? ($('.ab-test_button-option-three_animated-bg').css('height', '100%'),
      $('.minus-button').css('opacity', '1'))
    : screenWidth < 991 &&
      $('.ab-test_button-option-three.first').hasClass('opened-dd')
    ? ($('.ab-test_button-option-three_animated-bg').css('height', 'auto'),
      $('.ab-test_button-option-three_animated-bg').css('width', 'auto'))
    : screenWidth < 991 &&
      !$('.ab-test_button-option-three.first').hasClass('opened-dd')
    ? ($('.ab-test_button-option-three_animated-bg').css('height', '0px'),
      $('.ab-test_button-option-three_animated-bg').css('width', '100%'))
    : screenWidth > 991 &&
      !$('.ab-test_button-option-three.first').hasClass('opened-dd') &&
      ($('.ab-test_button-option-three_animated-bg').css('height', '0%'),
      $('.ab-test_button-option-three_animated-bg').css('width', '0%')),
    (responsiveAnimationHeight = $(
      '.ab-test-cf-animation-wrapper'
    ).outerHeight());
  if (screenWidth > 991) {
