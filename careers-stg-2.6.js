  ...
  if ($(window).width() > 991) {
    ...
    $(window).scroll(function () {
      ...
      if ($(".fadeup1").isInViewport()) {
        ...
        c.play();
        var legendItems = $(".careers_chart-legend")
        ...
        if (!viewedCareersGraph) {
          ...
          document
            ...
            .addEventListener("click", toggleDataFirst);
          document
            .querySelector("#second-chart")
            .addEventListener("click", toggleDataSecond);
          document
            .querySelector("#third-chart")
            .addEventListener("click", toggleDataThird);
          document
            .querySelector("#all-chart")
            .addEventListener("click", toggleDataAll);
          dragDetector();
    ...
    $.each(data, function (i, item) {
      if (item.jobs.length > 0) {
        ...
        $.each(item.jobs, function (i, jobs) {
          let teamJobGrid = $(
            '<div class="positions_grid-job-list"></div>'
          ).appendTo(teamWrap);
          $(
...
var ourTeamsDrag = new Dragdealer("our-team-drag", {
  ...
  animationCallback: function (x, y) {
    ...
    $(".our-teams_slider").css(
      "margin-left",
      -x * (ourTeamsSlider * totalTeams - screenWidth + 32)
    );
  },
});
...
function dragDetector() {
  var screenWidth = $(window).width();
  var totalTeams = $(".our-teams_slider").find(
