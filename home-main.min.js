...
function animationsRender() {
  if ($(window).width() > 991) {
    var t,
      e = anime.timeline({ loop: !1, autoplay: !1 });
    e.add({
    ...
    var i = anime.timeline({ loop: !1, autoplay: !1 });
    i.add({
      ...
      $(window).scroll(function () {
        (t = $(this).scrollTop()),
          ...
          $('.fadeup1').isInViewport()
            ? s.play()
            : $('.home-benefits_image-title').isInViewport()
            ? n || (new Chart(ctx, config), (n = !0))
            : $('.fadeup5').isInViewport()
            ? (i.play(),
              setTimeout(function () {
                $('.past-future_slider-slide').each(function (t) {
                  var e = $(this).find('._8_fundcards');
                  setTimeout(function () {
                    e.click();
                  }, 100 * t);
                });
              }, 500))
            : $('.fadeuppe').isInViewport() && o.play();
      });
  } else new Chart(ctx, config);
}
animationsRender();
var screenWidth = $(window).width(),
  closedFundsWrapper = Math.round($('.past-future_content').outerWidth()),
  closedFundSlidesN = $('.past-future_slider-mask').find(
    '.past-future_slider-slide'
  ).length,
  closedFundSlidesOW = Math.round(
    $('.past-future_slider-mask').find('.past-future_slider-slide').outerWidth()
  ),
  closedFundsTotalWidth = Math.round(
    ...
    animationCallback: function (t, e) {
      $('.past-future-slider-active_line').css(
        ...
        $('.past-future_slider').css(
          'margin-left',
          -t * (closedFundsTotalWidth - closedFundsWrapper + 32)
        );
    },
  });
$('.past-future_slider-wrap').on(
  ...
  function (t) {
    var e = (dd.getStep() + '').split(','),
      s = e[0],
      i = e[1];
    dd.setStep(parseInt(s) + 1, i);
  }
),
  $('.past-future_slider-wrap').on(
    'click',
...
var homeBenefitsImageWrap = $('.home-benefits-section').outerWidth(),
  sp500Width = $('.home-benefits_image').outerWidth(),
    ...
    animationCallback: function (t, e) {
      $('.home-benefits_drag-line-active').css(
        ...
        $('.home-benefits_image-wrap').css(
          'margin-left',
          -t * (sp500Width - homeBenefitsImageWrap + 128)
        );
    },
  });
function dragDetector() {
  var t = Math.round($('.closed-funds_ab-content').outerWidth()),
    e = Math.round($('.closed-funds_ab-wrapper-mask').outerWidth()),
    s = $('.closed-funds_ab-item').length;
  (abdd = new Dragdealer('ab-scroller', {
    ...
    horizontal: !0,
    vertical: !1,
    animationCallback: function (s, i) {
      $('.past-future-slider-active_line.ab-test').css(
        'width',
        Math.round(100 * s) + '%'
      ),
        $('.closed-funds_ab-wrapper-mask').css('margin-left', -s * (e - t));
    },
  })),
    t < e
      ? $('.past-future-drag-component.ab-test-component').css('opacity', '1')
      : $('.past-future-drag-component.ab-test-component').css('opacity', '0');
  var i = Math.round($('.past-future_content').outerWidth()),
    o = Math.round($('.past-future_slider-mask').outerWidth());
  dd = new Dragdealer('content-scroller', {
    steps: closedFundSlidesN,
    speed: 0.1,
    requestAnimationFrame: !0,
    ...
    animationCallback: function (t, e) {
      $('.past-future-slider-active_line').css(
        ...
        $('.past-future_slider').css('margin-left', -t * (o - i + 32));
    },
  });
  var n = Math.round($('.past-future_content').outerWidth()),
    r = Math.round($('.past-future_slider-mask').outerWidth());
  (dd = new Dragdealer('content-scroller', {
    steps: closedFundSlidesN,
    speed: 0.1,
    requestAnimationFrame: !0,
    ...
    animationCallback: function (t, e) {
      $('.past-future-slider-active_line').css(
        ...
        $('.past-future_slider').css('margin-left', -t * (r - n + 32));
    },
  })),
    n < r
      ? $('.past-future-drag-component').css('opacity', '1')
      : $('.past-future-drag-component').css('opacity', '0');
  var d = $('.home-benefits-section').outerWidth(),
    c = $('.home-benefits_image').outerWidth();
  (sp500slider = new Dragdealer('home-benefits-drag-tool', {
    speed: 0.1,
    requestAnimationFrame: !0,
    horizontal: !0,
    ...
    animationCallback: function (t, e) {
      $('.home-benefits_drag-line-active').css(
        ...
        $('.home-benefits_image-wrap').css('margin-left', -t * (c - d + 128));
    },
  })),
    d < c + 32
...
$('.book-demo').on('click', function () {
  ...
  $('.alt-single-press_wrap').each(function () {
    ...
    $(this).attr('data-testid', 'alt-single-press-wrap-' + t);
  }),
  $('.closed-funds_ab-item').each(function () {
    var t = $(this).index();
    $(this).attr('data-testid', 'closed-funds-card-ab' + t);
  }),
  $('.past-future_slider-slide').each(function () {
    var t = $(this).index();
    $(this)
      .find('.past-future_single-card')
      .attr('data-testid', 'closed-funds-card-' + t);
  }),
  ...
  $('.ab-test_button-option-three.first').click(function () {
    ...
    $(this).hasClass('opened-dd')
      ? (991 > $(window).width()
          ? ($(t).find('.pill-plus_icon').css('display', 'flex'),
            $(t).find('.pill-minus_icon').css('display', 'none'),
      ...
      : (991 > $(window).width()
          ? ($(t).find('.pill-plus_icon').css('display', 'none'),
            $(t).find('.pill-minus_icon').css('display', 'flex'),
  ...
  $('#cashflow-ab_test').click(function (t) {
    t.target === t.currentTarget &&
...
function checkCurrencies() {
  ...
  if ('IN' === getCookie('country-based'))
    for (var e = 0; e < t.length; e++)
      t[e].innerHTML.indexOf('€50,000') > -1 &&
        (t[e].innerHTML = t[e].innerHTML.replace('€50,000', '$60,000')),
        t[e].innerHTML.indexOf('€2 billion') > -1 &&
          (t[e].innerHTML = t[e].innerHTML.replace('€2 billion', '$2 billion'));
}
$(window).resize(function () {
  ...
  $('#cashflow-ab_test').click(function (t) {
    t.target === t.currentTarget && $('#cashflow-ab_test').fadeOut();
  }),
  checkCurrencies();
