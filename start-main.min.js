      ...
      $(window).scroll(function () {
        (e = $(this).scrollTop()),
          ...
          $('.fadeup1').isInViewport()
            ...
            ? s || (new Chart(ctx, config), (s = !0))
            : $('.fadeup5').isInViewport()
            ? (n.play(),
              setTimeout(function () {
                $('.past-future_slider-slide').each(function (e) {
                  var t = $(this).find('._8_fundcards');
                  setTimeout(function () {
                    t.click();
                  }, 100 * e);
                });
              }, 500))
            : $('.fadeuppe').isInViewport() && r.play();
      });
...
var screenWidth = $(window).width(),
  ...
  closedFundsTotalWidth = closedFundSlidesN * closedFundSlidesOW,
    ...
    animationCallback: function (e, t) {
      $('.past-future-slider-active_line').css(
        ...
        $('.past-future_slider').css(
          'margin-left',
          -e * (closedFundsTotalWidth - closedFundsWrapper + 32)
        );
    },
  });
$('.past-future_slider-wrap').on(
  ...
  function (e) {
    var t = (dd.getStep() + '').split(','),
      i = t[0],
      n = t[1];
    dd.setStep(parseInt(i) + 1, n);
  }
),
  $('.past-future_slider-wrap').on(
    'click',
    ...
    animationCallback: function (e, t) {
      $('.home-benefits_drag-line-active').css(
        ...
        $('.home-benefits_image-wrap').css(
          'margin-left',
          -e * (sp500Width - homeBenefitsImageWrap + 128)
        );
    },
  });
function dragDetector() {
  var e = Math.round($('.past-future_content').outerWidth()),
    t = Math.round($('.past-future_slider-mask').outerWidth());
  (dd = new Dragdealer('content-scroller', {
    steps: closedFundSlidesN,
    speed: 0.1,
    requestAnimationFrame: !0,
    horizontal: !0,
    vertical: !1,
    animationCallback: function (i, n) {
      $('.past-future-slider-active_line').css(
        'width',
        Math.round(100 * i) + '%'
      ),
        $('.past-future_slider').css('margin-left', -i * (t - e + 32));
    },
  })),
    e < t
      ? $('.past-future-drag-component').css('opacity', '1')
      : $('.past-future-drag-component').css('opacity', '0');
  var i = $('.home-benefits-section').outerWidth(),
    n = $('.home-benefits_image').outerWidth();
  (sp500slider = new Dragdealer('home-benefits-drag-tool', {
    speed: 0.1,
    requestAnimationFrame: !0,
    horizontal: !0,
    ...
    animationCallback: function (e, t) {
      $('.home-benefits_drag-line-active').css(
        ...
        $('.home-benefits_image-wrap').css('margin-left', -e * (n - i + 128));
    },
  })),
    i < n + 32
...
$('.book-demo').on('click', function () {
  ...
  $('.alt-single-press_wrap').each(function () {
    ...
    $(this).attr('data-testid', 'alt-single-press-wrap-' + e);
  }),
  $('.past-future_single-card').each(function () {
    var e = $(this).parent().index();
    $(this).attr('data-testid', 'closed-funds-card-' + e);
  }),
...
function checkCurrencies() {
  var e = document.getElementsByTagName('p'),
    t = getCookie('country-based');
  if ('US' === t)
    for (var i = 0; i < e.length; i++)
      e[i].innerHTML.indexOf('€50,000') > -1 &&
        (e[i].innerHTML = e[i].innerHTML.replace('€50,000', '$75,000')),
        e[i].innerHTML.indexOf('€2 billion') > -1 &&
          (e[i].innerHTML = e[i].innerHTML.replace('€2 billion', '$2 billion'));
  if ('CH' === t)
    for (var i = 0; i < e.length; i++)
      e[i].innerHTML.indexOf('€50,000') > -1 &&
        (e[i].innerHTML = e[i].innerHTML.replace('€50,000', 'CHF 50,000')),
        e[i].innerHTML.indexOf('€2 billion') > -1 &&
  ...
  if ('GB' === t)
    for (var i = 0; i < e.length; i++)
      e[i].innerHTML.indexOf('€50,000') > -1 &&
        (e[i].innerHTML = e[i].innerHTML.replace('€50,000', '\xa350,000')),
        e[i].innerHTML.indexOf('€2 billion') > -1 &&
          (e[i].innerHTML = e[i].innerHTML.replace(
            '€2 billion',
            '\xa31.7 billion'
  ...
  if ('HK' === t)
    for (var i = 0; i < e.length; i++)
      e[i].innerHTML.indexOf('€50,000') > -1 &&
        (e[i].innerHTML = e[i].innerHTML.replace('€50,000', '$60,000')),
        e[i].innerHTML.indexOf('€2 billion') > -1 &&
          (e[i].innerHTML = e[i].innerHTML.replace('€2 billion', '$2 billion'));
  if ('SG' === t)
    for (var i = 0; i < e.length; i++)
      e[i].innerHTML.indexOf('€50,000') > -1 &&
        (e[i].innerHTML = e[i].innerHTML.replace('€50,000', 'SGD 80,000')),
        e[i].innerHTML.indexOf('€2 billion') > -1 &&
          (e[i].innerHTML = e[i].innerHTML.replace(
            '€2 billion',
            'SGD 2.9 billion'
...
$('#start-email-input').focus(function () {
  (keepFocus = !0), $(this).addClass('start-input'), hideList();
}),
  $('start-email-form-submit').click(function () {
    (keepFocus = !0), hideList();
  }),
